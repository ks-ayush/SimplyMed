...
router.get("/details", async (req, res) => {
  try {
    const { lat, lng, destination_lat, destination_lng } = req.query;
    const cachevalue = await client.get("directions:" + lat + ":" + lng + ":" + destination_lat + ":" + destination_lng);
    if (cachevalue) {
    ...
    await client.set("directions:" + lat + ":" + lng + ":" + destination_lat + ":" + destination_lng, JSON.stringify(result));
    await client.expire("directions:" + lat + ":" + lng + ":" + destination_lat + ":" + destination_lng, 3600);
    res.json(result);
  } catch (error) {
